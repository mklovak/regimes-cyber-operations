...
df_2014 <- read_csv("outputs/df_model_2014.csv")
...
# ---- df_2014 overview ----
# Unit of observation: directed dyad-year (attacker -> victim, per year)
# Period: 2007-2014 (MID coverage limit)
# DV: Incident_Count_Clean — purely cyber, purely unprovoked operations
...
cat("============================================================\n")
cat("  df_2014 Overview (2007-2014, clean panel — primary)\n")
cat("============================================================\n")
cat("Total observations:", nrow(df_2014), "\n")
cat("Unique countries:", n_distinct(df_2014$attacker), "\n")
cat("Years covered:", length(unique(df_2014$Year)), "(", min(df_2014$Year), "-", max(df_2014$Year), ")\n")
cat("Dyad-years with at least one incident:", sum(df_2014$Incident_Count_Clean > 0), "\n")
cat("% non-zero:", round(sum(df_2014$Incident_Count_Clean > 0) / nrow(df_2014) * 100, 4), "%\n")
cat("Total incidents:", sum(df_2014$Incident_Count_Clean), "\n\n")
...
cat("============================================================\n")
cat(sprintf(
  "  df_2020: %d obs, %d incidents, 2007-2020, controls: W4 + GDP\n",
  nrow(df_2020), sum(df_2020$Incident_Count)
))
cat(sprintf(
  "  df_2016: %d obs, %d incidents, 2007-2016, controls: W4 + GDP + CINC\n",
  nrow(df_2016), sum(df_2016$Incident_Count)
))
cat(sprintf(
  "  df_2014: %d obs, %d incidents, 2007-2014, controls: W4 + GDP + CINC (clean DV)\n",
  nrow(df_2014), sum(df_2014$Incident_Count_Clean)
))
cat(sprintf(
  "  Incidents excluded in df_2014: %d (7 ACD + 40 MID-linked, 4 overlap)\n",
  sum(df_2016$Incident_Count[df_2016$Year <= 2014]) - sum(df_2014$Incident_Count_Clean)
))
cat(sprintf(
  "  Kinetic dyad-years dropped: %d\n\n",
  nrow(df_2016 %>% filter(Year <= 2014)) - nrow(df_2014)
))
...
##################### 1.4 Incident trend comparison (all three panels) #########

df_trend_compare <- bind_rows(
  df_2020 %>%
    group_by(Year) %>%
    summarise(total = sum(Incident_Count), .groups = "drop") %>%
    mutate(panel = "df_2020 (all)"),
  df_2016 %>%
    group_by(Year) %>%
    summarise(total = sum(Incident_Count), .groups = "drop") %>%
    mutate(panel = "df_2016 (all, CINC period)"),
  df_2014 %>%
    group_by(Year) %>%
    summarise(total = sum(Incident_Count_Clean), .groups = "drop") %>%
    mutate(panel = "df_2014 (clean, primary)")
)

p <- ggplot(df_trend_compare, aes(x = Year, y = total, color = panel)) +
  geom_line(linewidth = 1) +
  geom_point(size = 2) +
  labs(
    title = "Cyber Incidents per Year: Panel Comparison",
    subtitle = "Blue = all incidents (2007-2020). Orange = CINC period (2007-2016). Green = clean DV (2007-2014).",
    x = "Year", y = "Number of Incidents", color = NULL
  ) +
  scale_color_manual(values = c(
    "df_2020 (all)" = "steelblue",
    "df_2016 (all, CINC period)" = "darkorange",
...
##################### 1.5 Top 5 attackers — clean panel (df_2014) ##############

p <- df_2014 %>%
  group_by(attacker) %>%
  summarise(total = sum(Incident_Count_Clean)) %>%
  filter(total > 0) %>%
  top_n(5, total) %>%
  ggplot(aes(x = total, y = reorder(attacker, total))) +
  ...
  scale_x_continuous(expand = expansion(mult = c(0, 0.15)))
print(p)
ggsave(file.path(plot_dir, "11_top5_attackers_clean.png"), p, width = 8, height = 5)


##################### 1.6 Top 5 victims — clean panel (df_2014) ###############
...
p <- df_2014 %>%
  ...
  scale_x_continuous(expand = expansion(mult = c(0, 0.15)))
print(p)
ggsave(file.path(plot_dir, "12_top5_victims_clean.png"), p, width = 8, height = 5)
...
# 2.2a All countries (cross-sectional: mean W4 per country across panel years)
df_w4_all <- df_2020 %>%
  distinct(attacker, Year, .keep_all = TRUE) %>%
  group_by(attacker) %>%
  summarise(w4 = mean(attacker_w4), .groups = "drop") %>%
  rename(country = attacker)

cat("--- W4: All Countries (df_2020, cross-sectional) ---\n")
cat("Type: continuous\n")
cat("Level of measurement: ratio (0 = smallest coalition, 1 = largest)\n")
cat("Unique countries:", nrow(df_w4_all), "\n")
...
# 2.2b Non-zero attackers (countries that attacked at least once in 2007-2020)
...
cat("--- W4: Non-Zero Attackers (df_2020, cross-sectional) ---\n")
cat("Unique attacker countries:", nrow(df_w4_attackers), "\n")
...
# 2.2c Non-zero victims (countries targeted at least once in 2007-2020)
df_w4_victims <- df_2020 %>%
  ...
  inner_join(
    df_2020 %>%
      ...
      group_by(attacker) %>%
      summarise(w4 = mean(attacker_w4), .groups = "drop"),
    by = c("victim" = "attacker")
  ) %>%
  rename(country = victim)

cat("--- W4: Non-Zero Victims (df_2020, cross-sectional) ---\n")
cat("Unique victim countries:", nrow(df_w4_victims), "\n")
...
df_gdp_all <- df_2020 %>%
  distinct(attacker, Year, .keep_all = TRUE) %>%
  group_by(attacker) %>%
  summarise(ln_gdp_pc = mean(attacker_ln_gdp_pc), .groups = "drop") %>%
  rename(country = attacker)

cat("--- ln(GDP per capita): All Countries (df_2020, cross-sectional) ---\n")
cat("Type: continuous\n")
cat("Level of measurement: ratio (log-transformed GDP per capita in constant 2015 USD)\n")
cat("Unique countries:", nrow(df_gdp_all), "\n")
...
cat("--- ln(GDP per capita): Non-Zero Attackers (df_2020, cross-sectional) ---\n")
cat("Unique attacker countries:", nrow(df_gdp_attackers), "\n")
...
cat("--- ln(GDP per capita): Non-Zero Victims (df_2020, cross-sectional) ---\n")
cat("Unique victim countries:", nrow(df_gdp_victims), "\n")
...
p <- ggplot() +
  ...
  labs(
    ...
    subtitle = paste0(
      "Distribution of country-level mean ln(GDP per capita). All countries (n=",
      nrow(df_gdp_all), "), attackers (n=", nrow(df_gdp_attackers), "), victims (n=", nrow(df_gdp_victims), ")."
    ),
...
cat("--- W4: All Countries (df_2016, cross-sectional) ---\n")
cat("Type: continuous\n")
cat("Level of measurement: ratio (0 = smallest coalition, 1 = largest)\n")
cat("Unique countries:", nrow(df_w4_all_2016), "\n")
...
df_cinc_all <- df_2016 %>%
  distinct(attacker, Year, .keep_all = TRUE) %>%
  group_by(attacker) %>%
  summarise(cinc = mean(attacker_cinc), .groups = "drop") %>%
  rename(country = attacker)

cat("--- CINC: All Countries (df_2016, cross-sectional) ---\n")
cat("Type: continuous\n")
cat("Level of measurement: ratio (share of world total national material capability, 0 to ~0.2)\n")
cat("Unique countries:", nrow(df_cinc_all), "\n")
...
cat("--- CINC: Non-Zero Attackers (df_2016, cross-sectional) ---\n")
cat("Unique attacker countries:", nrow(df_cinc_attackers), "\n")
...
cat("--- CINC: Non-Zero Victims (df_2016, cross-sectional) ---\n")
cat("Unique victim countries:", nrow(df_cinc_victims), "\n")
...
p <- ggplot() +
  ...
  labs(
    ...
    subtitle = paste0(
      "Distribution of country-level mean CINC. All countries (n=",
      nrow(df_cinc_all), "), attackers (n=", nrow(df_cinc_attackers), "), victims (n=", nrow(df_cinc_victims), ")."
    ),
...
p <- ggplot() +
  ...
  labs(
    ...
    subtitle = paste0(
      "Distribution of country-level mean ln(CINC). All countries (n=",
      nrow(df_cinc_all), "), attackers (n=", nrow(df_cinc_attackers), "), victims (n=", nrow(df_cinc_victims), ")."
    ),
...
print(cor.test(df_2020$attacker_w4, df_2020$Incident_Count))
cat("victim_w4 vs Incident_Count:\n")
print(cor.test(df_2020$victim_w4, df_2020$Incident_Count))

# 5.2 Controls vs Incident Count
cat("\n--- 5.2 Controls vs Incident_Count ---\n")
cat("attacker_ln_gdp_pc vs Incident_Count:\n")
print(cor.test(df_2020$attacker_ln_gdp_pc, df_2020$Incident_Count))
cat("victim_ln_gdp_pc vs Incident_Count:\n")
print(cor.test(df_2020$victim_ln_gdp_pc, df_2020$Incident_Count))
...
print(cor.test(df_2020$attacker_w4, df_2020$attacker_ln_gdp_pc))
cat("victim_w4 vs victim_ln_gdp_pc:\n")
print(cor.test(df_2020$victim_w4, df_2020$victim_ln_gdp_pc))
...
cat("attacker_w4 vs Incident_Count:\n")
print(cor.test(df_2016$attacker_w4, df_2016$Incident_Count))
cat("victim_w4 vs Incident_Count:\n")
print(cor.test(df_2016$victim_w4, df_2016$Incident_Count))

# 6.2 CINC vs Incident Count (both raw and log-transformed)
cat("\n--- 6.2 CINC vs Incident_Count ---\n")
cat("attacker_cinc (raw) vs Incident_Count:\n")
print(cor.test(df_2016$attacker_cinc, df_2016$Incident_Count))
cat("attacker_ln_cinc (log) vs Incident_Count (supplementary — raw CINC used in models):\n")
print(cor.test(df_2016$attacker_ln_cinc, df_2016$Incident_Count))
cat("victim_cinc (raw) vs Incident_Count:\n")
print(cor.test(df_2016$victim_cinc, df_2016$Incident_Count))

# 6.3 Collinearity: W4 vs CINC
cat("\n--- 6.3 Collinearity: W4 vs CINC ---\n")
cat("attacker_w4 vs attacker_cinc:\n")
print(cor.test(df_2016$attacker_w4, df_2016$attacker_cinc))
cat("attacker_w4 vs attacker_ln_cinc (supplementary):\n")
print(cor.test(df_2016$attacker_w4, df_2016$attacker_ln_cinc))
cat("victim_w4 vs victim_cinc:\n")
print(cor.test(df_2016$victim_w4, df_2016$victim_cinc))
...
print(cor.test(df_cinc_gdp$cinc, df_cinc_gdp$ln_gdp_pc))

cat("ln(CINC) vs ln(GDP per capita) (supplementary):\n")
df_cinc_gdp <- df_cinc_gdp %>% mutate(ln_cinc = log(cinc + 0.0001))
print(cor.test(df_cinc_gdp$ln_cinc, df_cinc_gdp$ln_gdp_pc))
...
cat("--- attacker_w4 by incident occurrence ---\n")
print(t.test(attacker_w4 ~ has_incident, data = df_2016))

cat("--- victim_w4 by incident occurrence ---\n")
print(t.test(victim_w4 ~ has_incident, data = df_2016))

cat("--- attacker_cinc by incident occurrence ---\n")
print(t.test(attacker_cinc ~ has_incident, data = df_2016))

cat("--- victim_cinc by incident occurrence ---\n")
print(t.test(victim_cinc ~ has_incident, data = df_2016))


################################################################################
#                 PART 9: VARIABLE DESCRIPTIONS (df_2014 — clean panel)
################################################################################
# The primary dataset: purely cyber, purely unprovoked operations.
# DV: Incident_Count_Clean (7 ACD + 40 MID-linked excluded)
# Panel: 122 kinetic dyad-years dropped (no false zeros)

cat("\n============================================================\n")
cat("  Variable Descriptions: df_2014 (clean panel — primary)\n")
cat("============================================================\n\n")

# --- 9.1 Dependent variable: Incident_Count_Clean ---
cat("--- Incident_Count_Clean (Dependent Variable, Primary) ---\n")
cat("Type: discrete (count)\n")
cat("Level of measurement: ratio (meaningful zero — no incidents)\n")
cat("Construction: DCID 2.0 incidents (2007-2014), excluding:\n")
cat("  - 7 ACD incidents (cyber-to-cyber retaliation per Tallinn Manual)\n")
cat("  - 40 incidents in dyad-years with MID hostility >= 4 (use of force)\n")
cat("  - 122 kinetic dyad-years dropped from panel entirely\n")
summary(df_2014$Incident_Count_Clean)
cat("Variance:", var(df_2014$Incident_Count_Clean), "\n")
cat("Standard deviation:", sd(df_2014$Incident_Count_Clean), "\n")
cat("Skewness:", skewness(df_2014$Incident_Count_Clean), "\n")
cat("Zero-inflation:", round(mean(df_2014$Incident_Count_Clean == 0) * 100, 2), "%\n")
cat("Variance >> Mean confirms overdispersion.\n\n")

cat("Non-zero incident counts (df_2014 clean):\n")
df_2014 %>%
  filter(Incident_Count_Clean > 0) %>%
  count(Incident_Count_Clean, name = "Dyad_Years") %>%
  print(n = Inf)

# Compare DV distributions across panels
cat("\n--- DV Comparison Across Panels ---\n")
cat(sprintf(
  "  df_2020: mean = %.6f, var = %.6f, var/mean = %.1f\n",
  mean(df_2020$Incident_Count), var(df_2020$Incident_Count),
  var(df_2020$Incident_Count) / mean(df_2020$Incident_Count)
))
cat(sprintf(
  "  df_2016: mean = %.6f, var = %.6f, var/mean = %.1f\n",
  mean(df_2016$Incident_Count), var(df_2016$Incident_Count),
  var(df_2016$Incident_Count) / mean(df_2016$Incident_Count)
))
cat(sprintf(
  "  df_2014: mean = %.6f, var = %.6f, var/mean = %.1f\n",
  mean(df_2014$Incident_Count_Clean), var(df_2014$Incident_Count_Clean),
  var(df_2014$Incident_Count_Clean) / mean(df_2014$Incident_Count_Clean)
))
cat("Overdispersion present in all panels (var/mean >> 1).\n\n")

# --- 9.2 W4 on clean panel ---
df_w4_all_2014 <- df_2014 %>%
  distinct(attacker, Year, .keep_all = TRUE) %>%
  group_by(attacker) %>%
  summarise(w4 = mean(attacker_w4), .groups = "drop") %>%
  rename(country = attacker)

df_w4_attackers_2014 <- df_2014 %>%
  filter(Incident_Count_Clean > 0) %>%
  distinct(attacker) %>%
  inner_join(
    df_2014 %>%
...
cat("--- W4: All Countries (df_2014, cross-sectional) ---\n")
cat("Unique countries:", nrow(df_w4_all_2014), "\n")
summary(df_w4_all_2014$w4)

cat("\n--- W4: Non-Zero Attackers (df_2014 clean, cross-sectional) ---\n")
cat("Unique attacker countries:", nrow(df_w4_attackers_2014), "\n")
summary(df_w4_attackers_2014$w4)
cat("\n")

# Density plot for clean panel
p <- ggplot() +
  geom_density(
    data = df_w4_all_2014,
    aes(x = w4, fill = "All Countries"), alpha = 0.4
  ...
  geom_density(
    data = df_w4_attackers_2014,
    aes(x = w4, fill = "Non-Zero Attackers (clean)"), alpha = 0.35
  ) +
  scale_fill_manual(
    values = c("All Countries" = "#2A9D8F", "Non-Zero Attackers (clean)" = "steelblue"),
    name = NULL
  ) +
  labs(
    title = "W4 Distribution: All Countries vs Clean-Panel Attackers (df_2014)",
    subtitle = paste0(
      "Distribution of country-level mean Winning Coalition Index. All countries (n=",
      nrow(df_w4_all_2014), "), clean-panel attackers (n=", nrow(df_w4_attackers_2014), ")."
    ),
    x = "W4 Score", y = "Density"
  ) +
  theme_minimal() +
...
# --- 9.3 CINC on clean panel ---
df_cinc_all_2014 <- df_2014 %>%
  distinct(attacker, Year, .keep_all = TRUE) %>%
  group_by(attacker) %>%
  summarise(cinc = mean(attacker_cinc), .groups = "drop") %>%
  rename(country = attacker)

df_cinc_attackers_2014 <- df_2014 %>%
  filter(Incident_Count_Clean > 0) %>%
  distinct(attacker) %>%
  inner_join(
    df_2014 %>%
...
cat("--- CINC: Non-Zero Attackers (df_2014 clean, cross-sectional) ---\n")
cat("Unique attacker countries:", nrow(df_cinc_attackers_2014), "\n")
summary(df_cinc_attackers_2014$cinc)
cat("\n")

# Structural zeros in clean panel
near_zero_threshold <- 0.001
n_near_zero_2014 <- df_cinc_all_2014 %>%
  filter(cinc < near_zero_threshold) %>%
  nrow()
cat(sprintf(
  "Countries with CINC < %.3f in df_2014: %d out of %d (%.1f%%)\n\n",
  near_zero_threshold, n_near_zero_2014,
  nrow(df_cinc_all_2014),
  n_near_zero_2014 / nrow(df_cinc_all_2014) * 100
))


################################################################################
#                 PART 10: CORRELATION TESTS (df_2014)
################################################################################

cat("\n============================================================\n")
cat("  Correlation Tests: df_2014 (clean panel)\n")
cat("============================================================\n\n")

# 10.1 Core: W4 vs Incident_Count_Clean
cat("--- 10.1 Core: W4 vs Incident_Count_Clean ---\n")
cat("attacker_w4 vs Incident_Count_Clean:\n")
print(cor.test(df_2014$attacker_w4, df_2014$Incident_Count_Clean))
cat("victim_w4 vs Incident_Count_Clean:\n")
print(cor.test(df_2014$victim_w4, df_2014$Incident_Count_Clean))

# 10.2 CINC vs Incident_Count_Clean
cat("\n--- 10.2 CINC vs Incident_Count_Clean ---\n")
cat("attacker_cinc vs Incident_Count_Clean:\n")
print(cor.test(df_2014$attacker_cinc, df_2014$Incident_Count_Clean))
cat("victim_cinc vs Incident_Count_Clean:\n")
print(cor.test(df_2014$victim_cinc, df_2014$Incident_Count_Clean))

# 10.3 GDP vs Incident_Count_Clean
cat("\n--- 10.3 GDP vs Incident_Count_Clean ---\n")
cat("attacker_ln_gdp_pc vs Incident_Count_Clean:\n")
print(cor.test(df_2014$attacker_ln_gdp_pc, df_2014$Incident_Count_Clean))
cat("victim_ln_gdp_pc vs Incident_Count_Clean:\n")
print(cor.test(df_2014$victim_ln_gdp_pc, df_2014$Incident_Count_Clean))

# 10.4 Compare core correlations across panels
cat("\n--- 10.4 W4-DV Correlation Comparison Across Panels ---\n")
r_2020 <- cor(df_2020$attacker_w4, df_2020$Incident_Count)
r_2016 <- cor(df_2016$attacker_w4, df_2016$Incident_Count)
r_2014 <- cor(df_2014$attacker_w4, df_2014$Incident_Count_Clean)
cat(sprintf("  df_2020: r = %.4f\n", r_2020))
cat(sprintf("  df_2016: r = %.4f\n", r_2016))
cat(sprintf("  df_2014: r = %.4f\n", r_2014))
cat("Direction consistent across all panels (negative).\n")
cat("Note: Pearson r understates with 99.95% zeros; regression is more informative.\n\n")


################################################################################
#                 PART 11: DIFFERENCE OF MEANS TESTS (df_2014)
################################################################################

cat("\n============================================================\n")
cat("  Difference of Means Tests: df_2014 (clean panel)\n")
cat("============================================================\n\n")

df_2014 <- df_2014 %>%
  mutate(has_incident = ifelse(Incident_Count_Clean > 0, "Yes", "No"))

cat("--- attacker_w4 by incident occurrence ---\n")
print(t.test(attacker_w4 ~ has_incident, data = df_2014))

cat("--- victim_w4 by incident occurrence ---\n")
print(t.test(victim_w4 ~ has_incident, data = df_2014))

cat("--- attacker_cinc by incident occurrence ---\n")
print(t.test(attacker_cinc ~ has_incident, data = df_2014))

cat("--- victim_cinc by incident occurrence ---\n")
print(t.test(victim_cinc ~ has_incident, data = df_2014))

cat("--- attacker_ln_gdp_pc by incident occurrence ---\n")
print(t.test(attacker_ln_gdp_pc ~ has_incident, data = df_2014))

cat("--- victim_ln_gdp_pc by incident occurrence ---\n")
print(t.test(victim_ln_gdp_pc ~ has_incident, data = df_2014))


################################################################################
#                 PART 12: SUMMARY OF DESCRIPTIVE FINDINGS
################################################################################
...
cat("--- Panel structure ---\n")
cat(sprintf(
  "  df_2020: %d obs, %d incidents, 2007-2020, W4 + GDP\n",
  nrow(df_2020), sum(df_2020$Incident_Count)
))
cat(sprintf(
  "  df_2016: %d obs, %d incidents, 2007-2016, W4 + GDP + CINC\n",
  nrow(df_2016), sum(df_2016$Incident_Count)
))
cat(sprintf(
  "  df_2014: %d obs, %d incidents, 2007-2014, W4 + GDP + CINC (clean DV)\n",
  nrow(df_2014), sum(df_2014$Incident_Count_Clean)
))
cat("  Primary specification: df_2014 (purely cyber, purely unprovoked)\n\n")
...
cat("genuinely independent controls measuring different dimensions of state capacity.\n\n")

cat("--- Correlations with Incident_Count ---\n")
cat(sprintf(
  "attacker_w4:        r = %.4f (df_2020), %.4f (df_2016), %.4f (df_2014)\n",
  r_2020, r_2016, r_2014
))
cat("Direction consistent across all panels (negative). Small but expected\n")
cat("given 99.95% zeros; regression recovers the relationship.\n\n")

cat("--- Difference of Means (t-tests) ---\n")
cat("attacker_w4:        Attackers have lower W4 than non-attackers across all panels\n")
cat("victim_w4:          Modest gap — absorbed by CINC in regressions\n")
cat("attacker_cinc:      Attackers ~13x higher CINC — capability matters\n")
cat("victim_cinc:        Victims have higher CINC — powerful states get targeted\n")
cat("attacker_ln_gdp_pc: Not significant (p>0.05) — wealth doesn't predict aggression\n")
cat("victim_ln_gdp_pc:   Victims ~2.7x richer — wealth creates targets\n\n")

cat("--- Key interpretation ---\n")
cat("GDP per capita does NOT predict who attacks but strongly predicts who gets\n")
cat("targeted. Economic development creates targets worth attacking (financial\n")
cat("systems, IP, critical infrastructure) but does not create motivation to\n")
cat("attack. The motivation channel is W4 (coalition size).\n\n")
cat("This pattern holds on the clean panel (df_2014): after removing kinetic-\n")
cat("conflict-linked operations and ACD incidents, the descriptive relationships\n")
cat("are consistent with the full sample, confirming that the cleaning procedure\n")
cat("does not introduce selection bias.\n\n")
...
cat("This motivates the ZINB specification: CINC in the inflation stage\n")
...
# --- Graph 2: Most active directed dyads (Panel A) ---
# Goes with the DV operationalisation subsection in the thesis: the
# directed-dyad unit of observation — which attacker -> victim pairs account
# for the observed operations.
p <- df_2020 %>%
...
# zero-inflation that motivates the NB/ZINB estimators. Log10 y-axis so the
# ~412k zero dyad-years and the handful of high counts are both legible.
count_dist <- df_2020 %>%
  count(Incident_Count, name = "dyad_years")
