...
##################### 3. Load W4 (winning coalition index) — master namespace ##
...
# Verify: DCID country names vs W4
...
# Map WDI country names to W4 (e.g. "Russian Federation" -> "Russia")
...
# Sources:
#   - WDI (IT.NET.USER.ZS) — % of individuals using the Internet for 173 countries
#   - North Korea: coded as 0% (no public internet access)
#   - Taiwan: hard-coded from ITU / Taiwan NCC estimates
#   - Zanzibar: uses Tanzania's values
...
# Reuse the same wdi_to_w4 mapping defined in the GDP section
...
nrow(df_inet_tw %>% filter(is.na(internet_pct))) # Taiwan Internet: missing years: 0
df_inet_tw <- df_inet_tw %>% filter(!is.na(internet_pct))

# Zanzibar — use Tanzania values (Zanzibar is part of Tanzania in WDI)
df_inet_tz <- df_inet_long %>%
    filter(country == "Tanzania") %>%
    mutate(country = "Zanzibar")

# Combine all internet data
# Remove NK placeholder from WDI (has 0s and NAs), replace with explicit 0s
df_inet_long <- df_inet_long %>% filter(country != "North Korea")

df_inet_all <- bind_rows(df_inet_long, df_inet_nk, df_inet_tw, df_inet_tz)

nrow(df_inet_all %>% filter(is.na(internet_pct))) # Internet: country-years with missing values: 510
df_inet_all <- df_inet_all %>% filter(!is.na(internet_pct))

# Verify that all W4 countries have Internet coverage
missing_inet <- setdiff(all_w4_countries, unique(df_inet_all$country))
cat("W4 countries missing from Internet data:", length(missing_inet), "\n") # 0

# Merge Internet coverage into df_model
df_model <- df_model %>%
...
##################### 8. Save df_model_2020.csv (GDP + Internet, 2007-2020) ####
# Filter to complete cases
complete <- complete.cases(
    df_model$attacker_w4, df_model$victim_w4,
    df_model$attacker_ln_gdp_pc, df_model$victim_ln_gdp_pc,
    df_model$attacker_internet, df_model$victim_internet
)

df_dropped_2020 <- df_model[!complete, ]
nrow(df_dropped_2020) # Rows with incomplete data: 34028
cat("Countries missing data (W4/GDP/Internet, 2007-2020):", n_distinct(df_dropped_2020$attacker[ # 25
    is.na(df_dropped_2020$attacker_w4) |
        is.na(df_dropped_2020$attacker_ln_gdp_pc) |
        is.na(df_dropped_2020$attacker_internet)
]), "\n")
...
# Kept: 392272 obs (205 cyber incidents) | Dropped: 34028 obs (0 cyber incidents)
...
##################### 9. Load CINC and save df_model_2016.csv ##################
...
# Build 2007-2016 dataset with CINC instead of GDP and Internet
...
cat("Outputs:\n")
cat("  outputs/df_model_2020.csv\n")
cat("  outputs/df_model_2016.csv\n")
